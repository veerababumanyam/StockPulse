...
test.describe("StockPulse Theme Integration Tests", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const isLoggedIn = await page
      .locator('[data-testid="navbar-user-menu"]')
      .isVisible();
    if (!isLoggedIn) {
  ...
  test("ThemeEngine Initialization", async ({ page }) => {
    ...
    const hasInitMessage = logs.some(
      (log) =>
  ...
  test("Theme Analytics Integration", async ({ page }) => {
    ...
    if (await analyticsSection.isVisible()) {
      ...
      const totalChanges = page.locator(
        '[data-testid="analytics-total-changes"]',
      );
      const mostUsedTheme = page.locator(
        '[data-testid="analytics-most-used-theme"]',
      );
  ...
  test("AI Recommendations Section", async ({ page }) => {
    ...
    const recommendationsSection = page.locator(
      '[data-testid="theme-recommendations"]',
    );
  ...
  test("Auto-Switch Functionality", async ({ page }) => {
    ...
    if (await autoSwitchToggle.isVisible()) {
      ...
      const description = page.locator(
        '[data-testid="auto-switch-description"]',
      );
  ...
  test("Theme Performance and Memory Leaks", async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      const theme =
        THEME_TEST_CONFIG.themes[i % THEME_TEST_CONFIG.themes.length];
  ...
  test("Accessibility Compliance", async ({ page }) => {
    ...
    const themeSelector = page.locator(
      '.settings-theme-selector [role="button"]:first-child',
    );
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    await page.goto("/settings");
    const defaultThemeButton = page.locator(
...
class ThemeTestUtils {
  ...
  static async waitForThemeChange(page, expectedTheme, timeout = 5000) {
    await page.waitForFunction(
      (theme) =>
        document.documentElement.getAttribute("data-theme")?.includes(theme),
      expectedTheme,
      { timeout },
    );
  }
