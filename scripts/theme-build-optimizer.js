...
class ThemeBuildOptimizer {
  ...
  async loadThemes() {
    ...
    try {
      // Load color palettes
      const colorPalettesContent = await fs.readFile(this.config.colorsFile, 'utf8');
  ...
  async optimizeThemes() {
    ...
    for (const [themeName, theme] of this.themes) {
      ...
      theme.compressionRatio = originalSize / Buffer.byteLength(optimizedCSS, 'utf8');
      
      const optimizedSize = Buffer.byteLength(optimizedCSS, 'utf8');
      this.optimizationStats.optimizedSize += optimizedSize;
      
      console.log(`   ✓ ${themeName}: ${originalSize}B → ${optimizedSize}B (${Math.round((1 - optimizedSize/originalSize) * 100)}% reduction)`);
    }
    
    this.optimizationStats.compressionRatio = this.optimizationStats.originalSize / this.optimizationStats.optimizedSize;
  ...
  async generateCombinedThemeBundle() {
    ...
    for (const [themeName, theme] of this.themes) {
      ...
      const variables = this.extractCSSVariables(theme.optimizedCSS || theme.css);
...
`;
  ...
  generateOptimizationReport() {
    ...
    for (const [themeName, theme] of this.themes) {
      const originalSize = Buffer.byteLength(theme.css, 'utf8');
      const optimizedSize = Buffer.byteLength(theme.optimizedCSS || theme.css, 'utf8');
  ...
  printSummary() {
    ...
    console.log(`   Validation: ${stats.validationPassed} passed, ${stats.validationFailed} failed`);
