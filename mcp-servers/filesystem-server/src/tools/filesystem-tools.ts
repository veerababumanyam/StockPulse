...
import { SecurityValidator, PathValidationError } from '../security/path-validator.js';
...
export async function handleBrowseDirectory(
  ...
  try {
    ...
    await context.securityValidator.validateDirectoryOperation(args.path, 'read');
    ...
    if (args.recursive) {
      const subdirs = listing.filter(item => item.type === 'directory' && item.accessible);
      for (const subdir of subdirs.slice(0, 5)) { // Limit recursive depth
        try {
          const subdirPath = path.join(args.path, subdir.name);
          const subdirListing = await context.securityValidator.getSecureDirectoryListing(
            subdirPath,
            args.includeHidden || false
          );
          (result as any)[`subdirectory_${subdir.name}`] = subdirListing;
        } catch (error) {
          context.logger.warn(`Failed to read subdirectory ${subdir.name}: ${error}`);
        }
      }
    }
...
export async function handleReadFile(
  ...
  try {
    ...
    const validatedPath = await context.securityValidator.validatePath(args.path);
...
export async function handleWriteFile(
  ...
  try {
    ...
    if (encoding === 'utf8') {
      const mimeType = mime.lookup(validatedPath) || undefined;
      sanitizedContent = context.securityValidator.sanitizeContent(args.content, mimeType);
    }
...
export async function handleDelete(
  ...
  try {
    ...
    if (stats.isDirectory()) {
      await context.securityValidator.validateDirectoryOperation(args.path, 'delete');
    } else {
      await context.securityValidator.validateFileOperation(args.path, 'delete');
    }
...
export async function handleGetInfo(
  ...
  try {
    ...
    const result: any = {
      ...
      mimeType: stats.isFile() ? mime.lookup(validatedPath) || 'application/octet-stream' : undefined
    };
    ...
    if (stats.isFile()) {
      result.isBinary = await context.securityValidator.isBinaryFile(validatedPath);
    }
...
export async function handleSearch(
  ...
  try {
    ...
    if (!config.features.enableSearch) {
      throw new PathValidationError('Search feature is disabled', 'FEATURE_DISABLED');
    }
    
    await context.securityValidator.validateDirectoryOperation(args.rootPath, 'read');
    
    const validatedPath = await context.securityValidator.validatePath(args.rootPath);
    const maxResults = Math.min(args.maxResults || 50, config.limits.maxFilesPerRequest);
    const maxDepth = Math.min(args.maxDepth || 5, config.security.maxDirectoryDepth);
