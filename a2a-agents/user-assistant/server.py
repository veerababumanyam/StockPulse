...
class UserAssistantAgent(A2AAgent):
    ...
    async def _analyze_user_intent(self, message: str, data: Dict) -> Dict:
        ...
        if any(word in message_lower for word in ["portfolio", "performance", "holdings", "positions"]):
            return {"type": "portfolio_analysis", "entity": self._extract_entity(message), "timeframe": "default"}
        elif any(word in message_lower for word in ["market", "news", "trends", "sector"]):
            return {"type": "market_research", "entity": self._extract_entity(message), "scope": "general"}
        elif any(word in message_lower for word in ["buy", "sell", "trade", "execute", "order"]):
            return {"type": "trade_execution", "entity": self._extract_entity(message), "action": self._extract_action(message)}
        elif any(word in message_lower for word in ["risk", "volatility", "exposure", "compliance"]):
            return {"type": "risk_assessment", "entity": self._extract_entity(message), "analysis_type": "comprehensive"}
        elif any(word in message_lower for word in ["technical", "chart", "indicators", "signals"]):
            return {"type": "technical_analysis", "entity": self._extract_entity(message), "indicators": "standard"}
        else:
    ...
    async def _route_to_portfolio_analysis(self, intent: Dict, task_id: str) -> Dict:
        ...
        return {
            ...
            "data": {
                ...
                "recommendations": ["Consider rebalancing tech allocation", "Increase defensive positions"]
            },
    ...
    async def _route_to_market_research(self, intent: Dict, task_id: str) -> Dict:
        ...
        return {
            ...
            "data": {
                ...
                "news_highlights": ["Fed maintains rates", "Tech earnings beat expectations"]
            },
    ...
    async def _route_to_risk_assessment(self, intent: Dict, task_id: str) -> Dict:
        ...
        return {
            ...
            "data": {
                ...
                "recommendations": ["Consider reducing tech concentration", "Add defensive assets"]
            },
    ...
    async def _comprehensive_portfolio_workflow(self, user_id: str, task_id: str) -> A2ATaskOutput:
        ...
        return A2ATaskOutput(
            ...
            data={
                ...
                "agents_involved": ["portfolio-manager", "market-researcher", "risk-manager"],
                "analysis": {
    ...
    async def _trade_execution_workflow(self, trade_data: Dict, task_id: str) -> A2ATaskOutput:
        ...
        return A2ATaskOutput(
            ...
            data={
                ...
                "execution_time": "2024-05-29T10:30:15Z",
                "agents_involved": ["trading-assistant", "risk-manager", "portfolio-manager"]
            },
            mode=OutputMode.TEXT
        )
    
    async def _investment_research_workflow(self, research_data: Dict, task_id: str) -> A2ATaskOutput:
        ...
        return A2ATaskOutput(
            ...
            data={
                ...
                "price_target": 175.00,
                "agents_involved": ["market-researcher", "technical-analyst", "risk-manager"]
            },
            mode=OutputMode.TEXT
        )
    ...
    async def _prioritize_responses(self, responses: List[Dict]) -> Dict:
        ...
        prioritized = sorted(responses, key=lambda x: x.get("confidence", 0), reverse=True)
        return {"prioritized_responses": prioritized, "primary_recommendation": prioritized[0] if prioritized else None}
    
    async def _resolve_conflicts(self, responses: List[Dict]) -> Dict:
        """Resolve conflicting recommendations from different agents."""
        # Simple conflict resolution
        return {"resolution": "consensus_moderate", "rationale": "Taking average of conflicting recommendations"}
    
    async def _merge_insights(self, responses: List[Dict]) -> Dict:
        """Merge insights from multiple agents."""
        return {"merged_insights": "Combined analysis shows balanced outlook with moderate risk", "confidence": 7.5}
    ...
    async def _get_user_preferences(self, user_id: str) -> Dict:
        ...
        # Would call MCP auth/redis server
        return {"risk_tolerance": "moderate", "investment_style": "balanced", "notification_preferences": {"email": True}}
