...
import {
  allTools,
  toolHandlers,
  FileSystemToolsContext,
} from "./tools/filesystem-tools.js";
import {
  listResources,
  readResource,
  FileSystemResourcesContext,
} from "./resources/filesystem-resources.js";
...
class FileSystemMCPServer {
  ...
  constructor() {
    ...
    try {
      ...
      console.error(
        "DEBUG: Creating MCP Server with config:",
  ...
  try {
    ...
    if (configFile && (await fs.pathExists(configFile))) {
      try {
        ...
        console.error(
          `DEBUG: Failed to load config file ${configFile}:`,
          error,
        );
      }
    }
