...
export const handlers: RequestHandler[] = [
  ...
  http.post(
    ...
    async ({ request }) => {
      // MSW v2 uses { request } or individual destructured props
      // const body = await request.json() as FraudRequestBody; // Example if you need to read body

      const scenario =
        sessionStorage.getItem("MOCK_FRAUD_SCENARIO") || "low-risk";
  ...
  http.post(`${API_BASE_URL}/auth/register`, async ({ request }) => {
    ...
    const scenario =
      sessionStorage.getItem("MOCK_REGISTRATION_SCENARIO") || "success";
    const userEmailForTesting =
      sessionStorage.getItem("MOCK_REGISTRATION_EMAIL") || email;

    if (
      scenario === "emailExists" ||
      userEmailForTesting === "exists@example.com"
    ) {
      return HttpResponse.json(
    ...
    if (scenario === "error") {
      return HttpResponse.json(
        {
          message:
            "Mock: An internal server error occurred during registration.",
        },
        { status: 500 },
      );
    }
...
export const setMockFraudScenario = (
  scenario: "low-risk" | "medium-risk" | "high-risk" | "error" | null,
) => {
  if (scenario === null) sessionStorage.removeItem("MOCK_FRAUD_SCENARIO");
  else sessionStorage.setItem("MOCK_FRAUD_SCENARIO", scenario);
};

export const setMockRegistrationScenario = (
  scenario: "success" | "emailExists" | "error" | null,
  email?: string,
) => {
  if (scenario === null)
    sessionStorage.removeItem("MOCK_REGISTRATION_SCENARIO");
