...
export interface GridLayoutActions {
  ...
  getOptimalPosition: (type: WidgetType, breakpoint?: string) => { x: number; y: number };
  validateLayout: () => boolean;
  getLayoutStats: () => { totalWidgets: number; coverage: number; conflicts: number };
}
...
export const useGridLayout = (options: UseGridLayoutOptions = {}): [GridLayoutState, GridLayoutActions] => {
  const {
    initialWidgets = [],
    initialLayouts = {},
    persistLayout = true,
  ...
  const getOptimalPosition = useCallback((type: WidgetType, breakpoint = currentBreakpoint): { x: number; y: number } => {
    const currentLayout = layouts[breakpoint] || [];
    const cols = GRID_CONFIG.cols[breakpoint as keyof typeof GRID_CONFIG.cols] || 12;
    const widgetSize = WIDGET_SIZES[type][breakpoint] || WIDGET_SIZES[type].lg || { w: 4, h: 3 };
  ...
  // Add widget
  const addWidget = useCallback((type: WidgetType, position?: { x: number; y: number }): string => {
    ...
    Object.keys(GRID_CONFIG.breakpoints).forEach(breakpoint => {
      const size = WIDGET_SIZES[type][breakpoint] || WIDGET_SIZES[type].lg || { w: 4, h: 3 };
  ...
  // Duplicate widget
  const duplicateWidget = useCallback((widgetId: string): string => {
  ...
  const saveLayout = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem(`widget-layout-${layoutId}`, JSON.stringify(layoutData));
      return true;
    } catch (error) {
  ...
  const getLayoutStats = useCallback(() => {
    ...
    const cols = GRID_CONFIG.cols[currentBreakpoint as keyof typeof GRID_CONFIG.cols] || 12;
    ...
    const usedCells = currentLayout.reduce((sum, item) => sum + (item.w * item.h), 0);
    const coverage = totalCells > 0 ? (usedCells / totalCells) * 100 : 0;
    
    // Count conflicts
    const conflicts = currentLayout.reduce((count, item, index) => {
      const hasConflict = currentLayout.slice(index + 1).some(other =>
        !(item.x >= other.x + other.w ||
          item.x + item.w <= other.x ||
          item.y >= other.y + other.h ||
          item.y + item.h <= other.y)
