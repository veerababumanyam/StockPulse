...
import { X, Search, CheckSquare, Square, PlusCircle, Filter } from 'lucide-react';
...
const ALL_CATEGORIES: WidgetCategory[] = ['portfolio', 'market', 'trading', 'analytics', 'news'];
...
const WidgetLibrary: React.FC<WidgetLibraryProps> = ({
    ...
    if (filters.searchQuery) {
      widgets = widgets.filter(
        (widget) =>
          ...
          widget.tags?.some(tag => tag.toLowerCase().includes(filters.searchQuery.toLowerCase()))
      );
    }

    // Filter by category
    if (filters.categories.length > 0) {
      widgets = widgets.filter((widget) => filters.categories.includes(widget.category));
    }
  ...
  const groupedWidgets = useMemo(() => {
    ...
    ALL_CATEGORIES.forEach(category => {
      const categoryWidgets = filteredAndSortedWidgets.filter(w => w.category === category);
      if (categoryWidgets.length > 0) {
        const categoryMeta = WIDGET_LIBRARY.find(w => w.category === category); // Get a sample for name/icon
        groups.push({
          category: category,
    ...
    if (filters.categories.length === 0 && !filters.searchQuery && groups.length > 1) {
        ...
        return [{
            ...
            widgets: filteredAndSortedWidgets,
            isExpanded: true,
        }, ...groups]; // Show "All" then categorized
    }


    return groups.length > 0 ? groups : [{ // Ensure at least one group if results exist, or an empty state
        category: 'no-results' as WidgetCategory,
        name: filters.searchQuery || filters.categories.length > 0 ? 'No Matching Widgets' : 'No Widgets Available',
  ...
  const handleSortChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newSortBy = e.target.value as 'name' | 'category' | 'popularity' | 'recent';
      ...
      <div style={modalStyle(isDarkMode)}>
        ...
        <div style={controlsContainerStyle}>
            <div style={searchContainerStyle}>
                <Search size={18} style={{ color: 'var(--color-text-tertiary)', marginRight: 'var(--spacing-xs)' }} />
                <input
                    type="text"
                ...
                <div style={filterGroupStyle}>
                    ...
                    {ALL_CATEGORIES.map((category) => (
                        <button
                            ...
                            style={categoryButtonStyle(isDarkMode, filters.categories.includes(category))}
                        >
                        {filters.categories.includes(category) ? <CheckSquare size={16} style={{marginRight: 'var(--spacing-xxs)'}}/> : <Square size={16} style={{marginRight: 'var(--spacing-xxs)'}}/>}
                 ...
                 <div style={filterGroupStyle}>
                    ...
                    <div style={{display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)'}}>
                        <select value={filters.sortBy} onChange={handleSortChange} style={selectStyle(isDarkMode)}>
                            <option value="category">Category</option>
                            <option value="name">Name</option>
                            {/* <option value="popularity">Popularity</option> */}
                            {/* <option value="recent">Recently Added</option> */}
                        </select>
                        <button onClick={toggleSortOrder} style={sortOrderButtonStyle(isDarkMode)}>
                ...
                <div style={filterGroupStyle}>
                    <h4 style={filterGroupTitleStyle}>Options</h4>
                     <label style={checkboxLabelStyle(isDarkMode)}>
                        ...
                        onChange={() => setFilters(prev => ({ ...prev, showPremiumOnly: !prev.showPremiumOnly }))}
        ...
        <div style={widgetListStyle}>
          {groupedWidgets.map((group) => (
            group.widgets.length > 0 || group.category === ('no-results' as WidgetCategory) ? (
                  ...
                  {group.widgets.map((widget) => (
                    <div key={widget.type} style={widgetItemStyle(isDarkMode)}>
                      <div style={{display: 'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                        <div>
                            <h4 style={widgetTitleStyle(isDarkMode)}>{widget.name}</h4>
                            <p style={widgetDescriptionStyle(isDarkMode)}>{widget.description}</p>
                        </div>
                        <button
                      ...
                      <div style={widgetMetaStyle(isDarkMode)}>
                        ...
                        {widget.isPremium && <span style={premiumTagStyle(isDarkMode)}>Premium</span>}
                      </div>
                    </div>
                  ))}
                </div>
...
const filterButtonStyle = (isDarkMode: boolean, isActive: boolean): React.CSSProperties => ({
    display: 'flex',
    alignItems: 'center',
    padding: 'var(--spacing-sm) var(--spacing-md)',
...
const categoryButtonStyle = (isDarkMode: boolean, isActive: boolean): React.CSSProperties => ({
    display: 'flex',
    alignItems: 'center',
