      ...
      {[...Array(4)].map((_, index) => (
        <div key={index} className="bg-card rounded-lg border p-6 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Skeleton
              className="h-3 w-20"
              aria-label="Loading metric subtitle"
            />
          </div>
        </div>
      ))}
...
export const PositionsListSkeleton: React.FC<{ count?: number }> = ({
  count = 5,
}) => {
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Skeleton
          ...
          aria-label="Loading sort button"
        />
      </div>
      {[...Array(count)].map((_, index) => (
        ...
        >
          ...
          <div className="text-right space-y-2">
            <Skeleton
              className="h-5 w-20"
              aria-label="Loading position value"
            />
...
export const TransactionsSkeleton: React.FC<{ count?: number }> = ({
  count = 3,
}) => {
  return (
    <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Skeleton
                ...
                aria-label="Loading transaction type"
              />
              <Skeleton className="h-5 w-16" aria-label="Loading symbol" />
            </div>
            <Skeleton
              className="h-4 w-20"
              aria-label="Loading transaction date"
            />
          </div>
          <div className="text-right space-y-2">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Skeleton
          className="h-4 w-20"
          aria-label="Loading market status label"
        />
        <Skeleton
          className="h-5 w-16 rounded-full"
          aria-label="Loading market status badge"
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        {["S&P 500", "NASDAQ", "Dow Jones"].map((index) => (
          ...
          >
            <Skeleton
              className="h-4 w-16"
              aria-label={`Loading ${index} label`}
            />
            <div className="space-y-1">
              <Skeleton
                className="h-4 w-20"
                aria-label={`Loading ${index} value`}
              />
            </div>
          </div>
        ))}
...
export const AIInsightsSkeleton: React.FC<{ count?: number }> = ({
  count = 2,
}) => {
  return (
    <div className="space-y-4">
      <div className="text-center py-6">
        <Skeleton
          ...
          aria-label="Loading AI icon"
        />
        <Skeleton
          className="h-4 w-48 mx-auto"
          aria-label="Loading AI message"
        />
      </div>
      {count > 0 && (
        <div className="space-y-4">
          {[...Array(count)].map((_, index) => (
            <div key={index} className="border rounded-lg p-3">
              <div className="flex items-start justify-between mb-2">
                <Skeleton
                  className="h-4 w-32"
                  aria-label="Loading insight title"
                />
                <Skeleton
                  className="h-4 w-12 rounded-full"
                  aria-label="Loading insight priority"
                />
              </div>
              <Skeleton
                className="h-4 w-full mb-2"
                aria-label="Loading insight content line 1"
              />
              <Skeleton
                className="h-4 w-3/4 mb-2"
                aria-label="Loading insight content line 2"
              />
              <Skeleton
                className="h-3 w-20"
...
export const ChartSkeleton: React.FC<{
  ...
  showLegend?: boolean;
  type?: "line" | "bar" | "pie";
}> = ({ height = 200, showLegend = false, type = "line" }) => {
  return (
    <div className="space-y-3">
      {showLegend && (
          ...
          {[...Array(3)].map((_, index) => (
            <div key={index} className="flex items-center gap-2">
              <Skeleton
                ...
                aria-label="Loading legend color"
              />
              <Skeleton
                className="h-3 w-12"
                aria-label="Loading legend label"
              />
            </div>
          ))}
        </div>
...
export const TableSkeleton: React.FC<{
  ...
  rows?: number;
  showHeader?: boolean;
}> = ({ columns = 4, rows = 5, showHeader = true }) => {
  return (
    <div className="space-y-3">
      {showHeader && (
        <div
            ...
            <div className="bg-card rounded-lg border p-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Skeleton
                    className="h-6 w-40"
                    aria-label="Loading portfolio overview title"
                  />
                  <Skeleton
                    className="h-4 w-60"
                    aria-label="Loading portfolio description"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Skeleton
                      className="h-4 w-24"
                      aria-label="Loading total invested label"
                    />
                    <Skeleton
                      className="h-8 w-32"
                      aria-label="Loading total invested value"
                    />
                  </div>
                  <div className="space-y-2">
                    <Skeleton
                      className="h-4 w-24"
                      aria-label="Loading current value label"
                    />
                    <Skeleton
                      className="h-8 w-32"
