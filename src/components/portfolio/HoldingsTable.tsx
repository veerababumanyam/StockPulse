...
const PositionRow: React.FC<{
...
}> = React.memo(({ position, realtimeData, enableQuickActions, compactMode, onAction }) => {
  const [showActions, setShowActions] = useState(false);
  
  const handleAction = useCallback((actionType: PositionAction['type']) => {
    onAction?.({
      type: actionType,
  ...
  const displayChangePercentage = realtimePrice?.changePercentage ?? position.dayChangePercentage;
...
export const HoldingsTable: React.FC<HoldingsTableProps> = React.memo(({
  ...
  const handleSort = useCallback((column: HoldingsSortBy) => {
    const newDirection: SortDirection = 
      tableState.sortBy === column && tableState.sortDirection === 'desc' ? 'asc' : 'desc';
  ...
  }, [tableState.sortBy, tableState.sortDirection, onUpdateTableState, onUpdateFilters]);
  ...
  const handleFilterChange = useCallback((key: keyof HoldingsFilter, value: any) => {
    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {tableState.totalItems} position{tableState.totalItems !== 1 ? 's' : ''} 
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button
            variant={tableState.filters.profitableOnly ? "default" : "outline"}
            size="sm"
            onClick={() => handleFilterChange('profitableOnly', !tableState.filters.profitableOnly)}
            className="gap-2"
          >
            <Filter size={14} />
            Profitable Only
        ...
        <div className="border rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-border">
              ...
              <tbody className="bg-background divide-y divide-border" role="rowgroup">
                {paginatedPositions.length === 0 ? (
                  <tr>
                    <td 
                      colSpan={enableQuickActions ? (compactMode ? 7 : 8) : (compactMode ? 6 : 7)} 
                      className="px-6 py-12 text-center text-muted-foreground"
                    >
                      {tableState.filters.searchTerm || tableState.filters.profitableOnly || tableState.filters.sector
        ...
        {tableState.totalPages > 1 && (
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">
                {startIndex + 1}-{Math.min(endIndex, tableState.totalItems)} of {tableState.totalItems}
              </span>
