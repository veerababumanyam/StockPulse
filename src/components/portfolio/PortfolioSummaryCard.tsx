      ...
      <CardContent>
        {(change !== undefined || changePercentage !== undefined) ? (
          ...
          <div className="space-y-1">
            <div className={cn("font-bold", compactMode ? "text-lg" : "text-2xl")}>
...
export const PortfolioSummaryCard: React.FC<PortfolioSummaryCardProps> = React.memo(({
  summary,
  isLoading = false,
  className,
  compactMode = false,
  ...
  const profitabilityRate = summary.openPositions > 0 
  ...
  const metrics = [
    ...
    },
    {
      ...
      color: profitabilityRate >= 60 ? 'green' : profitabilityRate >= 40 ? 'default' : 'red' as const,
    },
  ];

  if (isLoading) {
    return (
      <div className={cn("grid gap-4", compactMode ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-4", className)}>
        {metrics.map((_, index) => (
          <MetricCard
            key={index}
            title=""
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge 
          variant={summary.totalGainLossPercentage >= 0 ? "default" : "destructive"}
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div className="text-center">
                <div className="font-medium text-muted-foreground">Portfolio Return</div>
                <div className={cn(
                  "text-lg font-bold",
              ...
              <div className="text-center">
                <div className="font-medium text-muted-foreground">Day Performance</div>
                <div className={cn(
                  "text-lg font-bold",
                  summary.dayChangePercentage >= 0 ? "text-green-600" : "text-red-600"
                )}>
