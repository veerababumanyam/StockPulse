...
const ThemeWrapper: React.FC<ThemeWrapperProps> = ({ children }) => {
  ...
  const shouldApplyColorTheme = () => {
    ...
    const isAuthenticated = !PUBLIC_ROUTES.some(route => currentPath === route || currentPath.startsWith(route + '/'));
  ...
  React.useEffect(() => {
    ...
    if (applyColorTheme) {
      ...
      console.log(`🎨 Applied full color theme "${colorTheme}" to application page: ${location.pathname}`);
    } else {
      ...
      console.log(`🌐 Applied neutral styling to public page: ${location.pathname}`);
    }
    ...
    root.setAttribute('data-route-type', applyColorTheme ? 'application' : 'public');
