...
const generateMockFinancialData = (
  years = 5,
): { yearlyData: YearlyData[]; summary: SummaryData } => {
  ...
  // Current financials
  const operatingMargin = 0.15 + Math.random() * 0.1; // 15% - 25%
  ...
  const nwcPercent = 0.1 + Math.random() * 0.05; // 10% - 15% of revenue
  // Terminal values
  ...
  for (let i = 0; i <= years; i++) {
    ...
    data.push({
      ...
      discountedFcf: parseFloat(
        (fcf / Math.pow(1 + discountRate, i)).toFixed(1),
      ),
    });
  }
  ...
  const enterpriseValue =
    data.reduce((sum, year) => sum + year.discountedFcf, 0) +
...
const DCFCalculator: React.FC<DCFCalculatorProps> = ({
  ...
  useEffect(() => {
    ...
    const updatedYearlyData = data.yearlyData.map(
      (year: any, index: number) => {
        const discountedFcf =
          year.fcf / Math.pow(1 + discountRate / 100, index);
        return {
          ...year,
    ...
    const terminalValue =
      terminalFcf / (discountRate / 100 - terminalGrowthRate / 100);
    const discountedTerminalValue =
      terminalValue /
      Math.pow(1 + discountRate / 100, updatedYearlyData.length - 1);

    // Update summary
    updatedSummary.terminalValue = parseFloat(terminalValue.toFixed(1));
    updatedSummary.discountedTerminalValue = parseFloat(
      discountedTerminalValue.toFixed(1),
    );

    // Recalculate enterprise value
    const enterpriseValue =
      updatedYearlyData.reduce(
        (sum: number, year: any) => sum + year.discountedFcf,
        0,
      ) + discountedTerminalValue;
    updatedSummary.enterpriseValue = parseFloat(enterpriseValue.toFixed(1));

    // Recalculate equity value and fair value
    updatedSummary.equityValue = parseFloat(
      (enterpriseValue - updatedSummary.netDebt).toFixed(1),
    );
    updatedSummary.fairValue = parseFloat(
      (updatedSummary.equityValue / updatedSummary.sharesOutstanding).toFixed(
        2,
      ),
    );
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          ...
          <div className="bg-primary-50 dark:bg-primary-900/20 p-4 rounded-lg mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center">
                ...
                <span
                  className={`ml-1 text-sm font-medium ${getValuationStatus().color}`}
                >
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  ${data.summary.fairValue.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-300">
                ...
                >
                  {(
                    ((data.summary.fairValue - currentPrice) / currentPrice) *
                    100
                  ).toFixed(1)}
                  %
                </span>
            ...
            <div className="mt-4 pt-4 border-t border-primary-200 dark:border-primary-800">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  ${data.summary.enterpriseValue.toFixed(1)}M
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                ...
                <span className="font-medium">
                  ${data.summary.netDebt.toFixed(1)}M
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-sm text-gray-600 dark:text-gray-300">
                ...
                <span className="font-medium">
                  ${data.summary.equityValue.toFixed(1)}M
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-sm text-gray-600 dark:text-gray-300">
                ...
                <span className="font-medium">
                  {data.summary.sharesOutstanding.toFixed(1)}M
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-900/30 p-4 rounded-lg mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTerminalGrowthRate(parseFloat(e.target.value))
                  }
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
                />
                <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  ${data.summary.terminalValue.toFixed(1)}M
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-sm text-gray-600 dark:text-gray-300">
                ...
                <span className="font-medium">
                  {(
                    (data.summary.discountedTerminalValue /
                      data.summary.enterpriseValue) *
                    100
                  ).toFixed(1)}
                  %
                </span>
                ...
                <thead>
                  <tr>
                    ...
                    {[1, 2, 3, 4].map((g) => (
                      <th
                        key={g}
                        className="px-2 py-1 bg-gray-50 dark:bg-gray-900/50 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider"
                      >
                        {g}%
                      </th>
                    ))}
                  ...
                  {[8, 10, 12, 14].map((wacc, i) => (
                    ...
                    >
                      ...
                      {[1, 2, 3, 4].map((g) => {
                        ...
                        const multiplier =
                          wacc === discountRate && g === terminalGrowthRate
                            ? 1
                            : wacc < discountRate
                              ? 1 + (discountRate - wacc) * 0.05
                              : 1 - (wacc - discountRate) * 0.05;
                        const growthMultiplier =
                          g > terminalGrowthRate
                            ? 1 + (g - terminalGrowthRate) * 0.03
                            : 1 - (terminalGrowthRate - g) * 0.03;
                        const value =
                          data.summary.fairValue *
                          multiplier *
                          growthMultiplier;

                        const isHighlighted =
                          wacc === Math.round(discountRate) &&
                          g === Math.round(terminalGrowthRate);
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">
          ...
          The fair value estimate of ${data.summary.fairValue.toFixed(2)} is
          derived from projected free cash flows over the next {forecastYears}{" "}
          years, discounted at a WACC of {discountRate}% and assuming a terminal
          growth rate of {terminalGrowthRate}%.
          {data.summary.discountedTerminalValue / data.summary.enterpriseValue >
          0.7
