...
const generateMockOptionsData = (strike: number, expiryDays: number) => {
  ...
  for (let i = -5; i <= 5; i++) {
    ...
    callPrice = parseFloat(
      (callPrice + (Math.random() * 0.5 - 0.25)).toFixed(2),
    );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        const optionsData = generateMockOptionsData(
          price,
        ...
        const atmIndex = optionsData.strikes.findIndex(
          (strike: any) =>
            strike.strike === price ||
            (strike.strike > price &&
              optionsData.strikes[optionsData.strikes.indexOf(strike) - 1]
                ?.strike < price),
        );
            ...
            <thead>
              <tr>
                <th
                  ...
                  className="bg-gray-50 dark:bg-gray-900/50 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center border-b border-r border-gray-200 dark:border-gray-700"
                >
                  Calls
                </th>
                ...
                <th
                  colSpan={showGreeks ? 7 : 5}
                  className="bg-gray-50 dark:bg-gray-900/50 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center border-b border-l border-gray-200 dark:border-gray-700"
                >
                  Puts
                </th>
              </tr>
              ...
              <div className="bg-gray-50 dark:bg-gray-900/30 p-3 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      ${selectedOption[optionType].bid.toFixed(2)} / $
                      {selectedOption[optionType].ask.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
              ...
              <div className="bg-primary-50 dark:bg-primary-900/20 p-3 rounded-lg">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-300 mt-2">
                  {optionType === "call" ? (
                    <>
                      This{" "}
                      {selectedOption.call.itm
                        ? "in-the-money"
                        : "out-of-the-money"}{" "}
                      call option gives you the right to buy {symbol} at $
                      {selectedOption.strike.toFixed(2)} before{" "}
                      {data.expiryDate}.
                    ...
                    <>
                      This{" "}
                      {selectedOption.put.itm
                        ? "in-the-money"
                        : "out-of-the-money"}{" "}
                      put option gives you the right to sell {symbol} at $
                      {selectedOption.strike.toFixed(2)} before{" "}
                      {data.expiryDate}.
                      {selectedOption.strike > price
