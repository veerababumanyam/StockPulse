  ...
  switch (status) {
    case 'completed':
      return (
        <Badge
          variant="outline"
          className="text-xs border-success-emphasis text-success-fg bg-success-muted"
        >
    ...
    case 'pending':
      return (
        <Badge
          variant="outline"
          className="text-xs border-warning-emphasis text-warning-fg bg-warning-muted"
        >
    ...
    case 'failed':
      return (
        <Badge
          variant="outline"
          className="text-xs border-danger-emphasis text-danger-fg bg-danger-muted"
        >
    ...
    case 'cancelled':
      return (
        <Badge
          variant="outline"
          className="text-xs border-muted-foreground/50 text-muted-foreground bg-muted/30"
        >
...
const RecentTransactions: React.FC<WidgetComponentProps> = ({
  ...
  const [transactionsData, setTransactionsData] =
    useState<RecentTransactionsData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await apiClient.getRecentTransactionsData(
        transactionLimit,
        accountId,
      );
  ...
  const renderTransactionItem = (item: TransactionItem) => (
    <div
      key={item.id}
      className="flex items-start justify-between py-2.5 px-1 border-b border-border/50 last:border-b-0 hover:bg-muted/50 rounded-md"
    >
    ...
    <Card className={cn('h-full flex flex-col', config.className)}>
      <CardHeader className="pb-2 pt-3">
        ...
        <CardDescription className="text-xs text-muted-foreground pt-0.5">
          Showing last {transactionsData.transactions.length} transactions. Last
          updated: {new Date(transactionsData.lastUpdated).toLocaleTimeString()}
        </CardDescription>
      </CardHeader>
