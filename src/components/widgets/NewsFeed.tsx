...
import { Newspaper, RefreshCw, AlertCircle, ExternalLink, RadioTower, Tag, Filter, Search } from 'lucide-react';
...
const NewsFeed: React.FC<WidgetComponentProps> = ({
  ...
  const [debouncedKeywords, setDebouncedKeywords] = useState<string>(initialKeywords);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      if(keywords !== initialKeywords && onConfigChange && config) {
        onConfigChange(widgetId, { ...config, config: { ...config.config, keywords }});
      }
    }, 500);
  ...
  const renderArticleItem = (article: NewsArticle) => (
    <div key={article.id} className="py-3 border-b border-border/50 last:border-b-0">
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-grow">
          <a href={article.url} target="_blank" rel="noopener noreferrer" className="hover:underline">
            <h4 className="text-sm font-semibold text-foreground line-clamp-2 leading-snug">{article.title}</h4>
          </a>
          <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">
            {article.summary}
          </p>
          <div className="text-xs text-muted-foreground mt-1 flex items-center flex-wrap gap-x-2 gap-y-1">
            <span className="flex items-center"><RadioTower className="h-3 w-3 mr-1" /> {article.source}</span>
            ...
            {article.symbols && article.symbols.length > 0 && (
                <div className="flex items-center">
                    ...
                    {article.symbols.map(sym => <Badge key={sym} variant="secondary" className="mr-1 text-xs font-normal">{sym}</Badge>)}
                </div>
            )}
            <a href={article.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-primary hover:underline">
  ...
  if (isLoading && !newsData) { // Show full loading state only on initial load
    return (
         ...
         <CardHeader className="pb-2 pt-3">
            <div className="flex items-center justify-between">
                <CardTitle className="text-base font-semibold flex items-center text-danger-fg">
                 ...
                 <Button variant="ghost" size="icon" onClick={fetchData} title="Retry loading news" className="h-7 w-7">
      ...
      <CardHeader className="pb-2 pt-3">
        <div className="flex items-center justify-between mb-1.5">
            <CardTitle className="text-base font-semibold flex items-center">
             ...
             <Button variant="ghost" size="icon" onClick={fetchData} title="Refresh news" className="h-7 w-7">
        ...
        <CardDescription className="text-xs text-muted-foreground pt-1">
            Displaying {newsData?.articles.length || 0} articles. Last updated: {newsData ? new Date(newsData.lastRefreshed).toLocaleTimeString() : 'N/A'}
        </CardDescription>
      </CardHeader>
