...
import { SectorPerformanceData, SectorPerformanceItem } from '../../types/widget-data';
...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../ui/select'; // Assuming Select component
import { cn } from '../../utils/tailwind';

const TIMEFRAME_OPTIONS: SectorPerformanceData['timeframe'][] = ['1D', '1W', '1M', 'YTD'];
...
const SectorPerformance: React.FC<WidgetComponentProps> = ({
  ...
  const [performanceData, setPerformanceData] = useState<SectorPerformanceData | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  
  const initialTimeframe = config.config?.timeframe as SectorPerformanceData['timeframe'] || '1D';
  const [selectedTimeframe, setSelectedTimeframe] = useState<SectorPerformanceData['timeframe']>(initialTimeframe);

  const fetchData = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await apiClient.getSectorPerformanceData(selectedTimeframe);
  ...
  const handleTimeframeChange = (newTimeframe: SectorPerformanceData['timeframe']) => {
    setSelectedTimeframe(newTimeframe);
    if(onConfigChange && config) {
        onConfigChange(widgetId, { ...config, config: { ...config.config, timeframe: newTimeframe }});
    }
  };
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-popover text-popover-foreground p-2 rounded shadow-lg border border-border">
          ...
          <p className={cn("text-xs", data.changePercent >= 0 ? "text-success-fg" : "text-danger-fg")}>
  ...
  return (
    <Card className={cn("h-full flex flex-col", config.className)}>
      <CardHeader className="pb-2 pt-3">
        ...
        {isEditMode && (
            <Select value={selectedTimeframe} onValueChange={(value) => handleTimeframeChange(value as SectorPerformanceData['timeframe'])}>
        ...
        <CardDescription className="text-xs text-muted-foreground pt-1">
            Performance for {selectedTimeframe}. Last updated: {performanceData ? new Date(performanceData.lastUpdated).toLocaleTimeString() : 'N/A'}
        </CardDescription>
      </CardHeader>
      <CardContent className="flex-grow pr-0 pl-1 pb-2 pt-1">
        {(!performanceData || performanceData.sectors.length === 0) && !isLoading && (
        ...
        {performanceData && performanceData.sectors.length > 0 && (
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={performanceData.sectors} layout="vertical" margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? "var(--color-border-muted)" : "var(--color-border)"} horizontal={false} />
                    <XAxis 
                        type="number" 
                        stroke={isDarkMode ? "var(--color-text-muted)" : "var(--color-text-secondary)"} 
                        fontSize={10}
                    ...
                    <Bar dataKey="changePercent" radius={[0, 4, 4, 0]}>
                        {performanceData.sectors.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={entry.changePercent >= 0 ? 'var(--color-success-fg)' : 'var(--color-danger-fg)'} />
                        ))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
