...
const TopMovers: React.FC<WidgetComponentProps> = ({
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await apiClient.getTopMoversData(
        selectedMarket,
        moversCount,
      );
  ...
  const handleMarketChange = (newMark: string) => {
    ...
    if (onConfigChange && config) {
      onConfigChange(widgetId, {
        ...config,
        config: { ...config.config, market: newMark },
      });
    }
  };
  ...
  const renderMoverItem = (
    ...
    <div
      key={item.id}
      className="flex items-center justify-between py-2 px-1 border-b border-border/50 last:border-b-0 hover:bg-muted/50 rounded-md"
    >
    ...
    if (!items || items.length === 0) {
      return (
        <p className="text-xs text-muted-foreground text-center py-4">
          No {type}s found for this market.
        </p>
      );
    }
          ...
          moversData && (
            <Tabs defaultValue="gainers" className="h-full flex flex-col">
              <TabsList className="grid w-full grid-cols-3 h-8 mx-2 mt-1 mb-0.5">
                <TabsTrigger
                  value="gainers"
                  className="text-xs h-6 px-1 py-0.5"
                >
                  <TrendingUp className="h-3.5 w-3.5 mr-1" />
                  Gainers
                </TabsTrigger>
                <TabsTrigger value="losers" className="text-xs h-6 px-1 py-0.5">
                  <TrendingDown className="h-3.5 w-3.5 mr-1" />
                  Losers
                </TabsTrigger>
                <TabsTrigger value="active" className="text-xs h-6 px-1 py-0.5">
