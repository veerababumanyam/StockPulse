...
import { PerformanceMetricsData, PerformanceMetricItem } from '../../types/widget-data';
import { useTheme } from '../../contexts/ThemeContext';
import { apiClient } from '../../services/api';
import { TrendingUp, RefreshCw, AlertCircle, HelpCircle, ArrowUp, ArrowDown, Minus } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '../ui/card';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '../ui/tooltip'; // Assuming tooltip component
...
const PerformanceMetrics: React.FC<WidgetComponentProps> = ({
  ...
  const renderTrendIcon = (trend?: 'up' | 'down' | 'neutral') => {
    switch (trend) {
      case 'up': return <ArrowUp className="h-3.5 w-3.5 text-success-fg" />;
      case 'down': return <ArrowDown className="h-3.5 w-3.5 text-danger-fg" />;
  ...
  const renderMetricItem = (item: PerformanceMetricItem) => (
    <div key={item.id} className="flex justify-between items-center py-2.5 border-b border-border/50 last:border-b-0">
      <div className="flex items-center">
        ...
        {item.period && <span className="text-xs text-muted-foreground ml-1.5">({item.period})</span>}
      </div>
      <div className="flex items-center">
        {renderTrendIcon(item.trend)}
        <span className="text-sm font-semibold text-foreground ml-1.5">{item.value}</span>
      </div>
    </div>
  );
    ...
    <Card className={cn("h-full flex flex-col", config.className)}>
      <CardHeader className="pb-2 pt-3">
        ...
        <CardDescription className="text-xs text-muted-foreground pt-0.5">
           Metrics for portfolio {metricsData.portfolioId}, as of {new Date(metricsData.asOfDate).toLocaleDateString()}
        </CardDescription>
      </CardHeader>
