...
const PortfolioOverviewPreview: React.FC<{ isPreview?: boolean }> = ({ isPreview = true }) => {
  return (
    <div className="h-full w-full p-4 bg-white dark:bg-gray-800 rounded-lg border">
      <div className="flex items-center space-x-2 mb-3">
        <PieChart className="h-4 w-4 text-blue-600" />
        <h3 className="text-sm font-medium text-gray-900 dark:text-white">Portfolio Overview</h3>
      </div>
      
      <div className="space-y-3">
        <div>
          <p className="text-xs text-gray-500">Total Value</p>
          <p className="text-lg font-bold text-gray-900 dark:text-white">$125,750.50</p>
        </div>
