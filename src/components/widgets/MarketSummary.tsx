...
import { Landmark, RefreshCw, AlertCircle, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
const MarketSummary: React.FC<WidgetComponentProps> = ({
  ...
  const renderIndexCard = (index: MarketIndexData) => (
    <div key={index.id} className="p-3 rounded-lg bg-card-foreground/5 dark:bg-card-foreground/10 flex-1 min-w-[150px]">
      <p className="text-sm font-medium text-primary truncate" title={index.name}>{index.shortName || index.name}</p>
      <p className="text-lg font-bold text-foreground">{index.value.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
      <div className={cn(
        "text-xs flex items-center",
        index.change >= 0 ? 'text-success-fg' : 'text-danger-fg'
      )}>
        {index.change >= 0 ? <ArrowUpRight className="inline h-3.5 w-3.5 mr-0.5"/> : <ArrowDownRight className="inline h-3.5 w-3.5 mr-0.5"/>}
  ...
  return (
    <Card className={cn("h-full flex flex-col", config.className)}>
      ...
      <CardContent className="flex-grow">
        ...
        <p className="text-xs text-muted-foreground mt-3 text-right">Last updated: {new Date(summaryData.lastUpdated).toLocaleTimeString()}</p>
      </CardContent>
    </Card>
  );
};
