...
import { PortfolioChartData, PortfolioChartDatapoint } from '../../types/widget-data';
...
const TIMEFRAME_OPTIONS: PortfolioChartData['timeframe'][] = ['1D', '1W', '1M', '3M', '1Y', 'ALL'];
...
const PortfolioChart: React.FC<WidgetComponentProps> = ({
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState<PortfolioChartData['timeframe']>(config.config?.defaultTimeframe || '1M');

  const fetchData = useCallback(async (timeframe: PortfolioChartData['timeframe']) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const processedData = useMemo(() => {
    return chartData?.datapoints.map(p => ({
      ...p,
  ...
  const themeColors = {
    ...
    grid: isDarkMode ? 'var(--color-border-subtle)' : 'var(--color-border-subtle)',
    tooltipBg: isDarkMode ? 'var(--color-surface-overlay)' : 'var(--color-surface-overlay)',
    tooltipText: isDarkMode ? 'var(--color-text-primary)' : 'var(--color-text-primary)',
    referenceLine: isDarkMode ? 'var(--color-warning-fg)': 'var(--color-warning-fg)'
  };

  const handleTimeframeChange = (timeframe: PortfolioChartData['timeframe']) => {
    setSelectedTimeframe(timeframe);
    // Persist this choice in widget config if onConfigChange is implemented
    if (onConfigChange) {
      onConfigChange(widgetId, { ...config, config: { ...config.config, defaultTimeframe: timeframe } });
    }
  };
      ...
      <Card className={cn("h-full flex flex-col", config.className)}>
         <CardHeader className="pb-2 pt-3">
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => fetchData(selectedTimeframe)} title="Retry loading chart">
            ...
            <CardHeader className="pb-2 pt-3">
                <div className="flex items-center justify-between">
                    <CardTitle className="text-base font-semibold flex items-center">
                     ...
                     <button onClick={() => fetchData(selectedTimeframe)} title="Reload chart">
                        <RefreshCw className="h-4 w-4 text-muted-foreground hover:text-primary" />
                    </button>
                </div>
            </CardHeader>
      ...
      <CardHeader className="pb-2 pt-3">
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
                ...
                <button onClick={() => fetchData(selectedTimeframe)} title="Refresh chart" className="ml-2">
                    <RefreshCw className="h-4 w-4 text-muted-foreground hover:text-primary" />
                </button>
            </div>
        </div>
      </CardHeader>
      <CardContent className="flex-grow p-0 pt-2 pr-2"> {/* Adjusted padding for chart */} 
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={processedData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            />
            <Tooltip 
                contentStyle={{ backgroundColor: themeColors.tooltipBg, border: `1px solid ${themeColors.grid}`, borderRadius: 'var(--border-radius-sm)'}}
                labelStyle={{ color: themeColors.tooltipText, fontWeight: 'bold' }}
            ...
            {chartData.previousClose && selectedTimeframe === '1D' && (
                <ReferenceLine y={chartData.previousClose} label={{value: "Prev Close", position: "insideRight", fill:themeColors.referenceLine, fontSize: 10 }} stroke={themeColors.referenceLine} strokeDasharray="3 3" />
            )}
            <Line type="monotone" dataKey="value" stroke={themeColors.stroke} strokeWidth={2} dot={false} activeDot={{ r: 6 }} />
          </LineChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
