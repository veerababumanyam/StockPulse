...
const getImpactBadge = (impact?: EventImpact) => {
  ...
  switch (impact) {
    case 'high':
      colorClasses = 'bg-danger-muted text-danger-fg border-danger-emphasis';
      break;
    case 'medium':
      colorClasses = 'bg-warning-muted text-warning-fg border-warning-emphasis';
      break;
    case 'low':
      colorClasses = 'bg-info-muted text-info-fg border-info-emphasis';
      break;
  }
  return (
    <Badge
      variant="outline"
      className={cn(
        'text-xs px-1.5 py-0.5 font-normal capitalize',
        colorClasses,
      )}
    >
      {impact}
...
const EconomicCalendar: React.FC<WidgetComponentProps> = ({
  ...
  const [calendarData, setCalendarData] = useState<EconomicCalendarData | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (config.config?.impactLevels)
        filters.impactLevels = config.config.impactLevels;
      if (config.config?.eventTypes)
        filters.eventTypes = config.config.eventTypes;
      ...
      if (response.success && response.data) {
        ...
        setError(
          response.message || 'Failed to fetch economic calendar events.',
        );
      }
    } catch (err: any) {
  ...
  const handleDateRangeChange = (newRange: {
    ...
    if (onConfigChange && config) {
      onConfigChange(widgetId, {
        ...config,
        config: { ...config.config, dateRange: newRange },
      });
    }
  };
  ...
  const navigateDateRange = (direction: 'prev' | 'next') => {
    ...
    const diffDays =
      (currentEnd.getTime() - currentStart.getTime()) / (1000 * 3600 * 24) + 1;
  ...
  const groupedEvents = useMemo(() => {
    ...
    return calendarData.events.reduce(
      (acc, event) => {
  ...
  const renderEventItem = (item: CalendarEventItem) => (
    <div
      key={item.id}
      className="py-2.5 px-2 border-b border-border/50 last:border-b-0 hover:bg-muted/50 rounded-md"
    >
  ...
  return (
    <Card className={cn('h-full flex flex-col', config.className)}>
      <CardHeader className="pb-2 pt-3">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-6 px-2 text-xs w-[170px] justify-start text-left font-normal"
                >
        ...
        {isEditMode && (
          <p className="text-xs text-muted-foreground">
            Filters (country, impact, type) can be added via widget settings.
          </p>
        )}
      </CardHeader>
