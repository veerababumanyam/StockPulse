...
export class ThemeComposer {
  ...
  generateFromBrand(colors: BrandColors, name: string = 'custom'): ColorTheme {
    ...
    if (secondary) {
      Object.assign(lightTheme, this.generateSecondaryScale(secondary, 'light'));
  ...
  validateComposition(composition: ThemeComposition): boolean {
    try {
      ...
      // Validate variant
      const validVariants: ThemeVariant[] = ['default', 'compact', 'comfortable', 'accessible'];
  ...
  private generateColorScale(primary: string, mode: 'light' | 'dark'): Record<string, string> {
  ...
  private generateSecondaryScale(secondary: string, mode: 'light' | 'dark'): Record<string, string> {
    const scale: Record<string, string> = {};
  ...
  private generateAccentScale(accent: string, mode: 'light' | 'dark'): Record<string, string> {
    const scale: Record<string, string> = {};
  ...
  private adjustColor(color: string, level: number, mode: 'light' | 'dark'): string {
  ...
  private applyVariant(theme: any, variant: ThemeVariant): any {
    const variantMap = {
      ...
      comfortable: { ...theme, spacing: this.scaleSpacing(theme.spacing, 1.25) },
  ...
  private applyAccessibility(theme: any, accessibility?: AccessibilityOverrides): any {
    ...
    if (accessibility.largerText) {
      accessibleTheme.fontSize = this.scaleTypography(accessibleTheme.fontSize, 1.25);
    }
  ...
  private applyCustomizations(theme: any, customizations?: Partial<ThemeTokens>): any {
  ...
  private setupValidators(): void {
    ...
    this.validators.set('color', (value: string) => /^#[0-9A-F]{6}$/i.test(value));
    this.validators.set('spacing', (value: string) => /^\d+(\.\d+)?(px|rem|em)$/.test(value));
  }
