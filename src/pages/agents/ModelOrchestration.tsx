...
const ModelOrchestration: React.FC = () => {
  ...
  const [availableModels, setAvailableModels] = useState<Model[]>([
    {
      ...
      provider: 'anthropic',
      type: 'text',
      capabilities: ['market_analysis', 'sentiment_analysis', 'portfolio_optimization', 'risk_assessment', 'trading_signals'],
    ...
    },
    {
      ...
      provider: 'google',
      type: 'multimodal',
      capabilities: ['market_analysis', 'sentiment_analysis', 'portfolio_optimization', 'risk_assessment'],
      contextWindow: 32000,
      maxTokens: 2048,
      status: 'available',
      latency: 700,
    ...
    },
    {
      ...
      provider: 'meta',
      type: 'text',
      capabilities: ['market_analysis', 'sentiment_analysis', 'portfolio_optimization'],
      contextWindow: 8000,
      maxTokens: 2048,
      status: 'available',
      latency: 500,
  ...
  const setActiveModelForCapability = (capabilityId: string, modelId: string) => {
    orchestration.setActiveModel(capabilityId, modelId);
    
    setCapabilities(prev => prev.map(cap => 
      cap.id === capabilityId 
        ? { ...cap, activeModel: modelId } 
        : cap
    ));
  ...
  return (
    <div className="container mx-auto py-6 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Model Orchestration</h1>
        ...
        <Card>
          ...
          <CardFooter>
            <div className="text-sm text-muted-foreground">
              {availableModels.filter(m => m.status === 'available').length} models ready
            </div>
          </CardFooter>
        </Card>
          ...
          <CardFooter>
            <div className="text-sm text-muted-foreground">
              {orchestration.isOrchestrationEnabled ? (
                <Badge variant="success" className="text-xs">Orchestration Active</Badge>
              ) : (
                <Badge variant="outline" className="text-xs">Orchestration Disabled</Badge>
              )}
            </div>
          </CardFooter>
        </Card>
              ...
              {capabilities.map(capability => {
                ...
                return (
                  <TableRow key={capability.id}>
                      ...
                      {activeModel ? (
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Latency: {activeModel.latency}ms | Reliability: {(activeModel.reliability * 100).toFixed(1)}%
                          </div>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Progress value={95 + Math.random() * 5} className="h-2 w-20" />
                        <span className="text-sm">{(95 + Math.random() * 5).toFixed(1)}%</span>
                      </div>
                    </TableCell>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-2">
                      Type: {model.type} | Context: {model.contextWindow.toLocaleString()} tokens | Max Output: {model.maxTokens.toLocaleString()} tokens
                    </p>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        ...
                        if (supportedCapabilities.length > 0) {
                          ...
                          setActiveModelForCapability(supportedCapabilities[0].id, model.id);
                        }
                      }}
                    >
                      Set as Primary
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="border rounded-lg p-4">
              ...
              <div className="mt-4">
                ...
                <Progress value={1.5} className="h-1 bg-gray-200" indicatorClassName="bg-red-500" />
              </div>
            </div>
      ...
      <Dialog open={showFallbackDialog} onOpenChange={setShowFallbackDialog}>
        <DialogContent className="max-w-2xl">
              ...
              {editingFallbackChain.length === 0 ? (
                ...
                <div className="space-y-2">
                  {editingFallbackChain.map((modelId, index) => {
                    const model = getModelById(modelId);
                    return model ? (
                      <div key={modelId} className="flex items-center justify-between border-b pb-2">
                        <div className="flex items-center gap-2">
            ...
            {selectedCapability && (
              <div className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {getAvailableModelsForFallback(selectedCapability).map(model => (
          ...
          <DialogFooter>
            ...
            <Button 
              onClick={() => selectedCapability && updateFallbackChain(selectedCapability, editingFallbackChain)}
            >
              Save Fallback Chain
            </Button>
          </DialogFooter>
