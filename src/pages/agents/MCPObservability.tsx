...
const MCPObservability: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSystemMetrics(prev => ({
        ...
        networkOut: Math.max(0.1, prev.networkOut + (Math.random() * 0.3 - 0.15)),
      ...
      setConnections(prev => prev.map(conn => ({
        ...
        requestsPerMinute: Math.max(1, conn.requestsPerMinute + (Math.random() * 6 - 3)),
        errorRate: Math.max(0, conn.errorRate + (Math.random() * 0.4 - 0.2)),
        lastActivity: Math.random() > 0.7 ? new Date().toISOString() : conn.lastActivity,
      ...
      if (Math.random() > 0.7) {
        ...
        const logLevel = logLevels[Math.floor(Math.random() * (logLevels.length - 1))]; // Less chance of error
        const components = ['federation-client', 'capability-manager', 'request-handler', 'auth-service', 'data-processor'];
        ...
        const newLog = {
          ...
          level: logLevel,
          message: getRandomLogMessage(connection.serverName, logLevel, component),
          traceId: connection.traceId,
          spanId: `span-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
          component
        };
  ...
  const getRandomLogMessage = (serverName: string, level: string, component: string) => {
    const infoMessages = [
  ...
  return (
    <div className="container mx-auto py-6 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">MCP Observability</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardFooter>
            <div className="text-sm text-muted-foreground">
              CPU: {systemMetrics.cpuUsage.toFixed(1)}% | Memory: {systemMetrics.memoryUsage.toFixed(1)}%
            </div>
          </CardFooter>
        </Card>
      </div>
            ...
            <TableBody>
              {connections.map(connection => (
                <TableRow key={connection.id}>
                  ...
                  <TableCell>{connection.requestsPerMinute.toFixed(1)}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <span className={
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-sm font-medium">{systemMetrics.networkIn.toFixed(2)} MB/s</span>
              </div>
              <Progress 
                value={systemMetrics.networkIn * 10} 
                className="h-2" 
              />
            </div>
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-sm font-medium">{systemMetrics.networkOut.toFixed(2)} MB/s</span>
              </div>
              <Progress 
                value={systemMetrics.networkOut * 10} 
                className="h-2" 
              />
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardFooter className="flex justify-between border-t px-6 py-4">
            <div className="text-sm text-muted-foreground">
              ...
              {alerts.filter(alert => !alert.acknowledged).length} active alerts
            </div>
            <Button 
              variant="outline" 
              size="sm"
              ...
              <TableBody>
                {traceLogs.map(log => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>
                      <div className="font-mono text-xs truncate max-w-[100px]">{log.traceId}</div>
                    </TableCell>
                    <TableCell>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border rounded-lg p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Monitoring for unusual authentication patterns, access attempts, and security violations
              </p>
            </div>
            ...
            <div className="border rounded-lg p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Unusual latency patterns detected in Trading API MCP Server responses
              </p>
            </div>
            ...
            <div className="border rounded-lg p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Monitoring for unusual data patterns, format changes, and schema violations
              </p>
            </div>
          </div>
          ...
          <Alert className="mt-6">
            ...
            <AlertDescription>
              The anomaly detection system uses machine learning to identify unusual patterns in MCP operations.
              Detected anomalies are automatically logged and may trigger alerts based on severity.
            </AlertDescription>
          </Alert>
        </CardContent>
        <CardFooter className="flex justify-between border-t px-6 py-4">
