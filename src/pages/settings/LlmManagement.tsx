      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isAddingModel} onOpenChange={setIsAddingModel}>
          ...
          <DialogContent className="sm:max-w-[600px]">
            ...
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(handleAddModel)}
                className="space-y-6"
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {field.value &&
                          llmProviders.find((p) => p.id === field.value)
                            ?.description}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="e.g., GPT-4 for Trading Analysis"
                          {...field}
                        />
                      </FormControl>
                      <FormDescription>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Lower values are more deterministic, higher values
                          more creative
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          This model will be used as the primary model for all
                          AI operations
                        </FormDescription>
                      </div>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Models to use if this model fails or is unavailable
        ...
        <Dialog
          open={!!isEditingModel}
          onOpenChange={(open) => !open && setIsEditingModel(null)}
        >
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
            ...
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(handleUpdateModel)}
                className="space-y-6"
              >
                <FormField
                  control={form.control}
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="all">
            All Models ({configuredModels.length})
          </TabsTrigger>
          {llmProviders.map((provider) => (
            <TabsTrigger key={provider.id} value={provider.id}>
              {provider.name} (
              {
                configuredModels.filter((m) => m.providerId === provider.id)
                  .length
              }
              )
            </TabsTrigger>
          ...
          <Card>
                ...
                <TableBody>
                  {filteredModels.length === 0 ? (
                    <TableRow>
                      ...
                      >
                        No models configured. Add your first model to get
                        started.
                      </TableCell>
                    </TableRow>
                  ) : (
                    filteredModels.map((model) => (
                      <TableRow key={model.id}>
                          ...
                          {model.fallbackModels.length > 0 ? (
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-sm">
                                {getFallbackModelNames(model.fallbackModels)}
                              </span>
                            </div>
                          ) : (
                            <span className="text-sm text-muted-foreground">
                        ...
                        <TableCell>
                          <Badge
                            variant={model.isActive ? "success" : "destructive"}
                          >
            ...
            <CardFooter className="flex justify-between border-t px-6 py-4">
              <div className="text-sm text-muted-foreground">
                ...
                Configure models with different parameters for specific use
                cases
              </div>
              <Button
                variant="outline"
        ...
        <CardContent>
          <ul className="space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span className="text-sm">
                <strong>Temperature settings:</strong> Use lower temperatures
                (0.1-0.3) for factual tasks and higher temperatures (0.7-0.9)
                for creative tasks.
              </span>
            </li>
            <li className="flex items-start gap-2">
              ...
              <span className="text-sm">
                <strong>Fallback chains:</strong> Configure fallback models to
                ensure reliability during provider outages or rate limiting.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <div className="mt-0.5 text-green-600">
              ...
              <span className="text-sm">
                <strong>Cost optimization:</strong> Use more capable models for
                complex tasks and more efficient models for simpler tasks.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <div className="mt-0.5 text-green-600">
              ...
              <span className="text-sm">
                <strong>Context windows:</strong> Match model context windows to
                your use case requirements to optimize performance and cost.
              </span>
            </li>
          </ul>
        </CardContent>
