...
test.describe("Multi-Role Authentication Tests", () => {
  test.describe("Admin Role Tests", () => {
    ...
    test("admin should access dashboard with admin privileges", async ({
      page,
    }) => {
    ...
    test("admin should access all trading features", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="intraday-trading"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="longterm-trading"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="options-trading"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="positional-trading"]'),
      ).toBeVisible();
    });
    ...
    test("admin should access analytics and reports", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="system-analytics"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="user-analytics"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="trading-analytics"]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe("User Role Tests", () => {
    ...
    test("user should access dashboard with limited privileges", async ({
      page,
    }) => {
      await page.goto(`${TEST_CONFIG.baseUrl}/dashboard`);
    ...
    test("user should have limited trading access", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="portfolio-view"]'),
      ).toBeVisible();

      // Verify advanced features are restricted
      await expect(
        page.locator('[data-testid="advanced-analytics"]'),
      ).not.toBeVisible();
      await expect(
        page.locator('[data-testid="system-controls"]'),
      ).not.toBeVisible();
    });
  ...
  test.describe("Guest (Unauthenticated) Tests", () => {
    ...
    test("guest should be redirected from protected routes", async ({
      page,
    }) => {
  ...
  test.describe("Role Switching Tests", () => {
    test("should handle role switching between admin and user", async ({
      browser,
    }) => {
      ...
      await expect(
        adminPage.locator('[data-testid="admin-controls"]'),
      ).toBeVisible();

      // Test user access simultaneously
      await userPage.goto(`${TEST_CONFIG.baseUrl}/dashboard`);
      await expect(
        userPage.locator('[data-testid="admin-controls"]'),
      ).not.toBeVisible();
      await expect(
        userPage.locator('[data-testid="portfolio-summary"]'),
      ).toBeVisible();
    ...
    test("should maintain separate sessions for different roles", async ({
      browser,
    }) => {
  ...
  test.describe("Permission Boundary Tests", () => {
    test("admin should access all API endpoints", async ({ page }) => {
      ...
      const response = await page.request.get(
        `${TEST_CONFIG.apiUrl}/api/v1/admin/users`,
      );
      expect(response.status()).toBe(200);

      const systemResponse = await page.request.get(
        `${TEST_CONFIG.apiUrl}/api/v1/admin/system`,
      );
    ...
    test("user should have restricted API access", async ({ page }) => {
      ...
      const userResponse = await page.request.get(
        `${TEST_CONFIG.apiUrl}/api/v1/user/profile`,
      );
    ...
    test("guest should have no API access to protected endpoints", async ({
      page,
    }) => {
      test.use({ storageState: "playwright/.auth/guest.json" });
  ...
  test.describe("Session Management Tests", () => {
    test("should maintain admin session across page reloads", async ({
      page,
    }) => {
      test.use({ storageState: "playwright/.auth/admin.json" });
