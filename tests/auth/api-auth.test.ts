...
test.describe('API Authentication Tests', () => {
  ...
  test.describe('Health Check', () => {
    test('should respond to health check', async ({ request }) => {
      const response = await request.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.health}`);
  ...
  test.describe('Login API Endpoint', () => {
    ...
    test('should successfully login with valid admin credentials', async ({ request }) => {
      ...
      const response = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: {
    ...
    test('should successfully login with valid user credentials', async ({ request }) => {
      await delay(API_CONFIG.delays.betweenRequests);
    ...
    test('should validate required fields', async ({ request }) => {
      ...
      // Test missing email
      const responseNoEmail = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: {
          password: API_CONFIG.credentials.admin.password
        }
      });
      ...
      const responseNoPassword = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: {
          email: API_CONFIG.credentials.admin.email
        }
      });
      ...
      // Test empty request
      const responseEmpty = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: {}
      });
    ...
    test('should validate email format', async ({ request }) => {
      ...
      const responseData = await response.json();
      expect(responseData.detail[0].msg).toContain('value is not a valid email address');
    });
  });
  ...
  test.describe('Registration API Endpoint', () => {
    ...
    test('should register new user with valid data', async ({ request }) => {
      ...
      const response = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.register}`, {
        data: {
          email: uniqueEmail,
      ...
      expect(responseData.user.full_name).toBe(API_CONFIG.credentials.newUser.full_name);
      expect(responseData.user.role).toBe('user');
    ...
    test('should reject duplicate email registration', async ({ request }) => {
      ...
      expect(errorText.toLowerCase()).toMatch(/email.*already|duplicate|exists/);
    });
    ...
    test('should validate password strength', async ({ request }) => {
      ...
      const responseData = await response.json();
      const errorText = JSON.stringify(responseData);
      expect(errorText.toLowerCase()).toMatch(/password.*8.*character|ensure.*value.*least/);
    });
  });
  
  test.describe('Protected Endpoints', () => {
    
    test('should access user profile with valid authentication', async ({ request }) => {
      await delay(API_CONFIG.delays.betweenRequests);
      
      // Login first
      const loginResponse = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: {
          email: API_CONFIG.credentials.user.email,
          password: API_CONFIG.credentials.user.password
        }
      ...
      const profileResponse = await request.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.me}`, {
        headers: {
          'Cookie': cookies
        }
      });
    ...
    test('should reject access to user profile without authentication', async ({ request }) => {
      await delay(API_CONFIG.delays.betweenRequests);
      
      const response = await request.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.me}`);
      
      expect(response.status()).toBe(401);
    });
    
    test('should access admin endpoints with admin authentication', async ({ request }) => {
      await delay(API_CONFIG.delays.betweenRequests);
      ...
      const adminResponse = await request.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.adminPendingUsers}`, {
        headers: {
          'Cookie': cookies
        }
      });
    ...
    test('should reject user access to admin endpoints', async ({ request }) => {
      await delay(API_CONFIG.delays.betweenRequests);
  ...
  test.describe('Logout API Endpoint', () => {
    ...
    test('should successfully logout authenticated user', async ({ request }) => {
      ...
      // Now logout
      const logoutResponse = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.logout}`, {
        headers: {
          'Cookie': cookies
        }
      });
    ...
    test('should handle logout without authentication', async ({ request }) => {
      ...
      const response = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.logout}`);
  ...
  test.describe('Error Handling', () => {
    ...
    test('should return consistent error format across endpoints', async ({ request }) => {
      ...
      const loginError = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.login}`, {
        data: { email: 'invalid' }
      });
      ...
      const registerError = await request.post(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.register}`, {
        data: { email: 'invalid' }
      });
  ...
  test.describe('API Documentation Compliance', () => {
    ...
    test('should match OpenAPI specification', async ({ request }) => {
      const specResponse = await request.get(`${API_CONFIG.baseUrl}/openapi.json`);
