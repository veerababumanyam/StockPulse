...
describe('Watchlist Component', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(addSymbolToWatchlist).mockResolvedValue({ success: true, message: 'Added successfully' });
    vi.mocked(removeSymbolFromWatchlist).mockResolvedValue({ success: true, message: 'Removed successfully' });
    vi.mocked(validateSymbol).mockResolvedValue({ valid: true, message: 'Valid symbol' });
  ...
  describe('Basic Rendering', () => {
    ...
    it('displays loading state initially', async () => {
      // Mock loading state
      vi.mocked(getUserWatchlist).mockImplementation(() => new Promise(resolve => setTimeout(() => resolve(mockWatchlistData), 100)));
    ...
    it('displays percentage changes when enabled', async () => {
      render(
        <TestWrapper>
          <Watchlist 
            widgetId="test-widget"
            config={{ ...mockConfig, config: { ...mockConfig.config, showPercentage: true } }}
            isEditMode={false}
            onConfigChange={mockOnConfigChange}
          />
        </TestWrapper>
    ...
    it('hides percentage changes when disabled', async () => {
      render(
        <TestWrapper>
          <Watchlist 
            widgetId="test-widget"
            config={{ ...mockConfig, config: { ...mockConfig.config, showPercentage: false } }}
            isEditMode={false}
            onConfigChange={mockOnConfigChange}
          />
        </TestWrapper>
    ...
    it('displays volume when enabled', async () => {
      render(
        <TestWrapper>
          <Watchlist 
            widgetId="test-widget"
            config={{ ...mockConfig, config: { ...mockConfig.config, showVolume: true } }}
            isEditMode={false}
            onConfigChange={mockOnConfigChange}
          />
        </TestWrapper>
  ...
  describe('Real-time Updates', () => {
    ...
    it('updates prices when WebSocket data is received', async () => {
      ...
      vi.mocked(subscribeToWatchlist).mockImplementation((items: any, callback: any) => {
  ...
  describe('Error Handling', () => {
    ...
    it('handles WebSocket connection failures gracefully', async () => {
      vi.mocked(connectToMarketData).mockRejectedValue(new Error('WebSocket connection failed'));
  ...
  describe('Accessibility', () => {
    it('has proper ARIA labels for interactive elements', async () => {
      ...
      expect(screen.getByTitle('Add new stock to watchlist')).toBeInTheDocument();
      expect(screen.getByTitle('Retry loading watchlist')).toBeInTheDocument();
    });
  ...
  describe('Performance', () => {
    it('does not re-render unnecessarily when props do not change', async () => {
      ...
      expect(vi.mocked(getUserWatchlist).mock.calls.length).toBe(initialCallCount);
    });
