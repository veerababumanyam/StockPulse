...
import { webSocketService, WebSocketService, MarketDataUpdate } from '../../src/services/websocketService';
...
describe('WebSocketService', () => {
  ...
  describe('Subscription Management', () => {
    ...
    it('subscribes to multiple symbols from watchlist', () => {
      const callback = vi.fn();
      const unsubscribe = webSocketService.subscribeToWatchlist(mockWatchlistItems, callback);
  ...
  describe('Message Handling', () => {
    ...
    it('handles error messages', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Server error:', 'Server error occurred');
    ...
    it('handles unknown message types', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Unknown message type:', 'unknown');
    ...
    it('handles malformed JSON messages', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Failed to parse message:', expect.any(Error));
    ...
    it('handles callback errors gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Error in callback:', expect.any(Error));
  ...
  describe('Connection Status Tracking', () => {
    ...
    it('handles status callback errors gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Error in status callback:', expect.any(Error));
  ...
  describe('Reconnection Logic', () => {
    ...
    it('stops reconnection attempts after max attempts', async () => {
      ...
      vi.spyOn(webSocketService, 'connect').mockRejectedValue(new Error('Connection failed'));
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Max reconnection attempts reached');
  ...
  describe('Message Sending', () => {
    ...
    it('handles send when not connected', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketService] Cannot send message - not connected');
  ...
  describe('Edge Cases', () => {
    it('handles WebSocket constructor errors', async () => {
      ...
      await expect(webSocketService.connect()).rejects.toThrow('WebSocket creation failed');
      expect(webSocketService.getConnectionStatus()).toBe('error');
    });
