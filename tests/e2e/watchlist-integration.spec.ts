...
test.describe("Watchlist Widget Integration", () => {
  ...
  test.describe("Widget Rendering", () => {
    test("displays watchlist widget on dashboard", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-widget"]'),
      ).toBeVisible();

      // Check widget title
      await expect(
        page.locator('[data-testid="watchlist-title"]'),
      ).toContainText("Watchlist");

      // Check for add button
      await expect(
        page.locator('[data-testid="watchlist-add-button"]'),
      ).toBeVisible();
    });
    ...
    test("shows loading state initially", async ({ page }) => {
      ...
      // Check loading state
      await expect(
        page.locator('[data-testid="watchlist-loading"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="watchlist-loading"]'),
      ).toContainText("Loading watchlist...");
    });
    ...
    test("displays stock items when loaded", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-item-GOOGL"]'),
      ).toBeVisible();

      // Check stock data display
      await expect(
        page.locator('[data-testid="stock-symbol-AAPL"]'),
      ).toContainText("AAPL");
      await expect(
        page.locator('[data-testid="stock-name-AAPL"]'),
      ).toContainText("Apple Inc.");
      await expect(
        page.locator('[data-testid="stock-price-AAPL"]'),
      ).toContainText("$185.42");
      await expect(
        page.locator('[data-testid="stock-change-AAPL"]'),
      ).toContainText("+$2.34 (+1.28%)");
    });
    ...
    test("shows empty state when no items", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-empty"]'),
      ).toContainText("No stocks in your watchlist");
      await expect(
        page.locator('[data-testid="watchlist-empty"]'),
      ).toContainText("Add your first stock to get started");
    });
  });
  ...
  test.describe("Add Symbol Functionality", () => {
    test("opens add symbol modal on button click", async ({ page }) => {
      ...
      // Check modal is open
      await expect(
        page.locator('[data-testid="add-symbol-modal"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="add-symbol-title"]'),
      ).toContainText("Add Stock to Watchlist");
    ...
    test("validates symbol format", async ({ page }) => {
      ...
      // Check validation message
      await expect(page.locator('[data-testid="symbol-error"]')).toContainText(
        "Symbol must be 1-5 letters only",
      );
    });
    ...
    test("successfully adds a valid symbol", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="symbol-validation"]'),
      ).toContainText("TSLA is valid");
      ...
      ).toContainText("TSLA added to watchlist successfully");
  ...
  test.describe("Remove Symbol Functionality", () => {
    test("shows remove button on hover", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="remove-button-AAPL"]'),
      ).toBeVisible();
    });
    ...
    test("successfully removes a symbol", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="success-message"]'),
      ).toContainText("AAPL removed from watchlist successfully");
    });
  });
  ...
  test.describe("Navigation Functionality", () => {
    ...
    test("displays stock detail page with correct data", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="stock-symbol"]')).toContainText(
        "AAPL",
      );
      await expect(page.locator('[data-testid="stock-name"]')).toContainText(
        "Apple Inc.",
      );
      await expect(page.locator('[data-testid="stock-price"]')).toContainText(
        "$185.42",
      );
      await expect(page.locator('[data-testid="stock-change"]')).toContainText(
        "+$2.34 (+1.28%)",
      );
      await expect(
        page.locator('[data-testid="stock-pe-ratio"]'),
      ).toContainText("28.5");
      await expect(page.locator('[data-testid="stock-eps"]')).toContainText(
        "6.51",
      );
    });

    test("allows adding/removing from watchlist on stock detail page", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="remove-from-watchlist"]'),
      ).toBeVisible();
    });
  });
    ...
    test("simulates real-time price updates", async ({ page }) => {
      ...
      await page.evaluate(() => {
        ...
        window.dispatchEvent(
          new CustomEvent("mock-websocket-update", { detail: mockUpdate }),
        );
      });
      ...
      await expect(
        page.locator('[data-testid="stock-price-AAPL"]'),
      ).toContainText("$190.50");
      await expect(
        page.locator('[data-testid="stock-change-AAPL"]'),
      ).toContainText("+$7.42 (+4.05%)");
    });
  ...
  test.describe("Error Handling", () => {
    test("displays error state when API fails", async ({ page }) => {
      ...
      // Check error state
      await expect(
        page.locator('[data-testid="watchlist-error"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="watchlist-error"]'),
      ).toContainText("Error loading watchlist");
  ...
  test.describe("Accessibility", () => {
    ...
    test("has proper ARIA labels", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-add-button"]'),
      ).toHaveAttribute("aria-label", "Add stock to watchlist");
      await expect(
        page.locator('[data-testid="watchlist-refresh-button"]'),
      ).toHaveAttribute("aria-label", "Refresh watchlist data");
    });
    ...
    test("supports screen reader navigation", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-title"]'),
      ).toHaveAttribute("role", "heading");

      // Check for proper list structure
      await expect(
        page.locator('[data-testid="watchlist-items"]'),
  ...
  test.describe("Performance", () => {
    ...
    test("handles large watchlists efficiently", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="watchlist-item-SYM00"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="watchlist-item-SYM49"]'),
      ).toBeVisible();
