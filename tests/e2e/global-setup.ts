...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    const authResponse = await page.request.post('http://localhost:8000/api/v1/auth/login', {
      data: {
    ...
    console.log('   Please ensure both frontend and backend servers are running');
    console.log('   Frontend: npm run dev (port 3000)');
    console.log('   Backend: python -m uvicorn app.main:app --reload (port 8000)');
  } finally {
    await browser.close();
  }
