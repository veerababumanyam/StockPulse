...
const TestComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <button data-testid="check-status-btn" onClick={checkAuthStatus}>
        Check Status
      </button>
...
const TestStatusComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="status-anonymous">
...
const TestRequireAuthComponent: React.FC = () => {
  ...
  return (
    <div>
      <div data-testid="requires-auth">
        {requiresAuth ? "requires-auth" : "no-auth-required"}
      </div>
      <div data-testid="require-loading">
        {isLoading ? "loading" : "not-loading"}
      </div>
...
describe("AuthContext - Story 1.3 Tests", () => {
  ...
  describe("AC2: Login success updates authentication state", () => {
    it("should update context with user information when login succeeds", async () => {
      ...
      expect(mockAuthService.setCsrfToken).toHaveBeenCalledWith(
        "test-csrf-token",
      );
    });
  ...
  describe("Helper hooks", () => {
    it("should provide useAuthStatus hook with computed properties", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("status-loading")).toHaveTextContent(
          "not-loading",
        );
      });

      await waitFor(() => {
        expect(screen.getByTestId("status-authenticated")).toHaveTextContent(
          "authenticated",
        );
      });

      expect(screen.getByTestId("status-anonymous")).toHaveTextContent(
        "not-anonymous",
      );
      expect(screen.getByTestId("status-user")).toHaveTextContent(
        mockUser.email,
      );
    });
    ...
    it("should provide useRequireAuth hook for protected components", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("require-loading")).toHaveTextContent(
          "not-loading",
        );
      });

      await waitFor(() => {
        expect(screen.getByTestId("requires-auth")).toHaveTextContent(
          "requires-auth",
        );
      });

      expect(screen.getByTestId("require-error")).toHaveTextContent(
