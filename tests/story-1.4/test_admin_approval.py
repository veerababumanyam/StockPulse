        ...
        async for db in get_db():
            ...
            try:
                test_user = await user_service.create_user(
                    email="test@example.com", password="TestPass123!", db=db
                )
                ...
                if "already exists" in str(e):
                    ...
                    test_user = await user_service.get_user_by_email(
                        "test@example.com", db
                    )
                else:
                    raise
            ...
            admin_user = await user_service.get_user_by_email(
                "admin@stockpulse.com", db
            )
            if not admin_user:
            ...
            test_user_found = any(
                user.email == "test@example.com" for user in pending_users
            )
            ...
            try:
                approved_user = await user_service.approve_user(
                    str(test_user.id), str(admin_user.id), db
                )
            ...
            try:
                reject_user = await user_service.create_user(
                    email="reject@example.com", password="TestPass123!", db=db
                )
                print("   Created user for rejection test")
            except ValueError:
                reject_user = await user_service.get_user_by_email(
                    "reject@example.com", db
                )
                print("   Using existing user for rejection test")

            try:
                rejected_user = await user_service.reject_user(
                    str(reject_user.id), str(admin_user.id), "Test rejection", db
                )
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        if success:
            ...
            print("\n📝 Next steps:")
            print(
                "1. Start the backend server: cd services/backend && python -m uvicorn main:app --reload"
            )
