        ...
        try:
            ...
            if validation_result.is_valid:
                ...
                if validation_result.rate_limit_remaining:
                    print(f"   Rate limit remaining: {validation_result.rate_limit_remaining}")
            else:
                print(f"⚠️  FMP API key validation failed: {validation_result.error_message}")
            
            print("\n🚀 FMP API key setup completed successfully!")
            print("You can now use the StockPulse dashboard with real Financial Modeling Prep data.")
