    ...
    async def update_user_context(self, user_id: str, updates: Dict) -> None:
        ...
        try:
            ...
            if context:
                ...
                logger.warning(
                    "user_context_not_found_for_update",
                    user_id=user_id
                )
                
        except Exception as e:
            logger.error(
                "failed_to_update_user_context",
                user_id=user_id,
                error=str(e)
            )
            raise
    ...
    async def delete_user_context(self, user_id: str) -> None:
        ...
        try:
            if user_id in self._contexts:
                ...
                logger.info(
                    "user_context_deleted",
                    user_id=user_id
                )
            else:
                logger.warning(
                    "user_context_not_found_for_deletion",
                    user_id=user_id
                )
                
        except Exception as e:
            logger.error(
                "failed_to_delete_user_context",
                user_id=user_id,
                error=str(e)
            )
            raise
