...
class KnowledgeGraphService:
    ...
    async def initialize(self):
        ...
        try:
            if not GRAPHITI_AVAILABLE:
                logger.warning("Graphiti not available - knowledge graph features will be disabled")
                self.graphiti = None
                return
    ...
    async def add_correlation_relationship(
        ...
        try:
            ...
            logger.debug(f"Added correlation relationship: {symbol1} <-> {symbol2} ({correlation:.3f})")
            return True
    ...
    async def add_supply_chain_relationship(
        ...
        try:
            ...
            logger.debug(f"Added supply chain relationship: {supplier_symbol} -> {customer_symbol}")
            return True
            ...
            for record in results:
                ...
                if len(rels) == 1:
                    ...
                    relationships['direct_relationships'].append({
                        'type': rel.type,
                        'target': target_node.get('symbol') or target_node.get('name'),
    ...
    async def get_portfolio_risk_exposure(
        self,
        portfolio_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for sector, weight in sector_weights.items():
                risk_analysis['sector_exposure'][sector] = weight / total_weight if total_weight > 0 else 0
            ...
            if total_weight > 0:
                ...
                risk_analysis['diversification_score'] = 1 - risk_analysis['concentration_risk']
