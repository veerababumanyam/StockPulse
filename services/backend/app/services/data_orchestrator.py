...
class DataOrchestrator:
    ...
    async def get_market_data(
        ...
        if db_data and not self._is_data_stale(
            db_data, self.cache_strategies["market_data"]["ttl"]
        ):
    ...
    async def store_market_data(
        ...
        try:
            ...
            await self.cache_service.cache_market_data(
                symbol, data_type, data, self.cache_strategies["market_data"]["ttl"]
            )
            ...
            if data_type in ["news", "analysis", "insights"]:
                await self._store_market_data_in_vector(
                    symbol, data_type, data, metadata
                )

            # Update knowledge graph relationships
            await self._update_knowledge_graph_from_market_data(
                symbol, data_type, data, metadata
            )
    ...
    async def get_portfolio_data(
        ...
        # Try cache first
        if not force_refresh:
            cached_data = await self.cache_service.get_portfolio_data(
                user_id, data_type
            )
            if cached_data:
    ...
    async def store_portfolio_snapshot(
        ...
        try:
            ...
            snapshot = PortfolioSnapshot(
                ...
                total_gain_loss_percent=portfolio_data.get(
    ...
    async def store_news_article(
        ...
        try:
            ...
            await self.cache_service.set(
                "news", article_id, news_data, self.cache_strategies["news_data"]["ttl"]
            )
    ...
    async def analyze_portfolio_risk(
        self, db: AsyncSession, user_id: UUID, portfolio_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            kg_risk = await self.knowledge_graph_service.get_portfolio_risk_exposure(
                portfolio_id
            )
            ...
            for holding in holdings[:10]:  # Analyze top 10 holdings
                ...
                if symbol:
                    relationships = (
                        await self.knowledge_graph_service.get_company_relationships(
    ...
    async def get_market_insights(
        ...
        try:
            ...
            if symbols:
                for symbol in symbols:
                    influencers = (
                        await self.knowledge_graph_service.find_market_influencers(
                            symbol, limit=3
                        )
                    )
    ...
    async def optimize_data_layers(self, db: AsyncSession):
        ...
        try:
            ...
            for collection in self.vector_service.collections.keys():
                await self.vector_service.cleanup_old_embeddings(
                    collection, days_old=30
                )
    ...
    def _calculate_composite_risk_score(
        ...
        correlation_risk = len(
            [
                symbol
                for symbol, analysis in correlation_analysis.items()
    ...
    async def _fetch_fresh_market_data(
        ...
        try:
            ...
            logger.debug(
                f"Fresh market data fetch not implemented for {symbol}:{data_type}"
            )
            return None
        ...
        try:
            if data_type == "news":
                await self.vector_service.store_news_embedding(
                    ...
                    data.get("title", ""),
                    data.get("content", ""),
                    {"symbol": symbol, "data_type": data_type, **metadata},
                )
