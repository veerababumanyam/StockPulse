...
class CacheService:
    ...
    def _build_cache_key(
        self, cache_type: str, key: str, user_id: Optional[UUID] = None
    ) -> str:
    ...
    async def set(
        ...
        try:
            ...
            await self.metrics.record_set_operation(
                cache_type, cache_key, response_time, len(serialized_value)
            )
    ...
    async def extend_ttl(
        ...
        try:
            ...
            if current_ttl > 0:
                ...
                logger.debug(
                    f"Cache TTL extended: {cache_key} (+{additional_seconds}s)"
                )
                return result
            return False
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            if keys:
                ...
                logger.info(
                    f"Cache invalidated {deleted} keys matching pattern: {pattern}"
                )
                return deleted
            return 0
    ...
    async def get_portfolio_data(
        self, user_id: UUID, data_type: str
    ) -> Optional[Dict[str, Any]]:
...
class CacheMetricsCollector:
    ...
    async def record_hit(
        self, cache_type: str, cache_key: str, response_time_ms: float
    ):
        """Record cache hit"""
    ...
    async def record_set_operation(
        self, cache_type: str, cache_key: str, response_time_ms: float, size_bytes: int
    ):
        """Record cache set operation"""
        await self._record_metric(
            cache_type, cache_key, "set", response_time_ms, size_bytes
        )
