...
class SecurityService:
    ...
    async def log_security_event(
        ...
        if not db:
            ...
            print(
                f"SECURITY EVENT: {event_type} - Success: {success} - User: {user_id} - IP: {ip_address}"
            )
            return
        ...
        try:
            ...
            # If database logging fails, rollback and log to console
            await db.rollback()
            print(
                f"SECURITY EVENT (DB ERROR): {event_type} - Success: {success} - User: {user_id} - IP: {ip_address} - Error: {str(e)}"
            )
    ...
    async def clear_failed_login_attempts(self, email: str):
        ...
        # For now, just log it
        await self.log_security_event(
            "failed_attempts_cleared", success=True, details={"email": email}
        )
