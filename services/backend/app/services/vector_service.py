...
class VectorService:
    ...
    async def store_news_embedding(
        self, article_id: str, title: str, content: str, metadata: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            return await self.store_embedding(
                "market_news", article_id, embedding, payload
            )
    ...
    async def store_research_embedding(
        ...
        try:
            ...
            return await self.store_embedding(
                "company_research", research_id, embedding, payload
            )
    ...
    async def store_portfolio_embedding(
        ...
        try:
            ...
            metrics_text = ", ".join(
                [f"{key}: {value}" for key, value in performance_metrics.items()]
            )
            ...
            return await self.store_embedding(
                "portfolio_analysis", portfolio_id, embedding, payload
            )
    ...
    async def find_similar_portfolios(
        self, portfolio_id: str, limit: int = 10, exclude_user_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Find portfolios similar to the given portfolio"""
        try:
            # Get the portfolio embedding
            portfolio_info = self.client.retrieve(
                collection_name="portfolio_analysis", ids=[portfolio_id]
            )
            ...
            for result in results:
                if result["id"] != portfolio_id and (
                    not exclude_user_id
                    or result["payload"].get("user_id") != exclude_user_id
                ):
    ...
    async def store_ai_insight_embedding(
        ...
        try:
            ...
            return await self.store_embedding(
                "ai_insights", insight_id, embedding, payload
            )
    ...
    async def cleanup_old_embeddings(
        self, collection_name: str, days_old: int = 30
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cleanup requested for {collection_name} older than {days_old} days"
            )
            return 0
