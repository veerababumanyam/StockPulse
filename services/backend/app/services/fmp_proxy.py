...
class FMPProxyService:
    ...
    async def get_portfolio_overview(
        ...
        try:
            ...
            indices = await self._make_request(
                db, user_id, "quote/^GSPC,^DJI,^IXIC"
            )
    ...
    async def get_portfolio_chart_data(
        ...
        try:
            ...
            historical_data = await self._make_request(
                db, user_id, f"historical-chart/{interval}/SPY",
    ...
    async def get_watchlist_data(
        ...
        try:
            ...
            quotes = await self._make_request(
                db, user_id, f"quote/{symbol_string}"
            )
    ...
    async def get_sector_performance(
        ...
        try:
            # Get sector performance data
            sectors_data = await self._make_request(
                db, user_id, "sectors-performance"
            )
            ...
            for sector in sectors_data:
                sectors.append({
                    ...
                    "change_percent": float(sector.get("changesPercentage", "0").replace("%", "")),
    ...
    async def get_top_movers(
        self, db: AsyncSession, user_id: UUID, market: str = "nasdaq", type: str = "gainers"
    ) -> Dict[str, Any]:
        """Get top movers data"""
        try:
