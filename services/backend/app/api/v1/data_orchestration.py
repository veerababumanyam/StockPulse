...
async def get_data_orchestrator(
...
async def get_market_data(
    symbol: str,
    data_type: str = Query(
        "quote", description="Type of market data (quote, chart, news, fundamentals)"
    ),
...
async def search_related_content(
    ...
    content_types: List[str] = Query(
        ["news", "insights"], description="Types of content to search"
    ),
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"query": query, "results": results, "total_results": len(results)},
            "timestamp": datetime.utcnow().isoformat(),
        }
...
async def get_market_insights(
    ...
    insight_types: Optional[List[str]] = Query(
        None, description="Types of insights to retrieve"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: CurrentUser = Depends(get_current_user),
    orchestrator: DataOrchestrator = Depends(get_data_orchestrator),
