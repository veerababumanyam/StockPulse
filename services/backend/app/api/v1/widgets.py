...
async def _generate_widget_data(widget_type: str, config: Optional[Dict[str, Any]], user_id: UUID) -> Dict[str, Any]:
    """
    ...
    if widget_type == "portfolio-overview":
        ...
        return {
            "indices": [
                {"symbol": "S&P 500", "value": 4567.89, "change": 23.45, "change_percent": 0.52},
                {"symbol": "NASDAQ", "value": 14234.56, "change": -45.67, "change_percent": -0.32},
                {"symbol": "DOW", "value": 34567.12, "change": 156.78, "change_percent": 0.46},
                {"symbol": "VIX", "value": 18.45, "change": -1.23, "change_percent": -6.25}
            ],
            "last_updated": datetime.utcnow().isoformat()
        }
    
    elif widget_type == "watchlist":
        symbols = config.get("symbols", ["AAPL", "GOOGL", "MSFT", "TSLA"]) if config else ["AAPL", "GOOGL", "MSFT", "TSLA"]
        return {
            "stocks": [
                {"symbol": symbol, "price": 150.25 + i * 10, "change": 2.45 - i * 0.5, "change_percent": 1.65 - i * 0.3}
