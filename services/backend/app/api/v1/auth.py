...
from app.services.auth.jwt_service import jwt_service, csrf_service
...
async def login(
    ...
    try:
        ...
        # Authenticate user
        user = await user_service.authenticate_user(credentials.email, credentials.password, db)
        
        if not user:
            # Record failed attempt
            await security_service.record_failed_login_attempt(credentials.email, ip_address)
            await security_service.log_security_event(
                "login_failed",
                ip_address=ip_address,
