...
class WidgetInstance(BaseModel):
    ...
    """
    id: str = Field(..., description="Unique identifier for the widget instance (e.g., 'widget-portfolio-overview-123')")
    type: WidgetTypeLiterals = Field(..., description="The type of the widget.")
    layout: Dict[str, WidgetInstanceLayout] = Field(..., description="Layout position and dimensions for different breakpoints. Keys are breakpoint names (e.g., 'lg', 'md').")
    config: Dict[str, Any] = Field(default_factory=dict, description="Specific configuration for this widget instance.")

class DashboardLayout(BaseModel):
    """
    Defines the arrangement of widgets on the dashboard.
    """
    widgets: List[WidgetInstance] = Field(default_factory=list, description="List of widget instances on the dashboard.")

class DashboardConfigBase(BaseModel):
    """
    Base schema for dashboard configuration.
    """
    name: str = Field(default="My Dashboard", description="Name of the dashboard configuration.")
    layout: DashboardLayout = Field(default_factory=DashboardLayout, description="The layout of widgets on the dashboard.")
