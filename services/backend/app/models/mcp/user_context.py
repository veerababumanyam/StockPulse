...
class UserPreferences(BaseModel):
    ...
    ui_settings: Dict[str, Any] = Field(
        default_factory=dict, description="UI configuration"
    )
...
class PortfolioSettings(BaseModel):
    ...
    asset_allocation: Dict[str, float] = Field(
        default_factory=dict, description="Target asset allocation"
    )
    max_position_size: Decimal = Field(
        default=Decimal("0.1"), description="Maximum position size as percentage"
    )
    min_cash_reserve: Decimal = Field(
        default=Decimal("0.05"), description="Minimum cash reserve percentage"
    )
    ...
    def validate_rebalancing_frequency(cls, v):
        ...
        if v not in allowed_frequencies:
            raise ValueError(
                f"rebalancing_frequency must be one of {allowed_frequencies}"
            )
        return v
...
class RiskProfile(BaseModel):
    ...
    max_daily_loss: Decimal = Field(
        default=Decimal("0.02"), description="Maximum daily loss percentage"
    )
    max_portfolio_drawdown: Decimal = Field(
        default=Decimal("0.15"), description="Maximum portfolio drawdown"
    )
    position_sizing_method: str = Field(
        default="fixed_percentage", description="Position sizing methodology"
    )
...
class TradingStrategy(BaseModel):
    ...
    risk_limits: Dict[str, Decimal] = Field(
        default_factory=dict, description="Risk limits for strategy"
    )
...
class UserContext(BaseModel):
    ...
    portfolio_settings: PortfolioSettings = Field(
        ..., description="Portfolio configuration"
    )
    risk_profile: RiskProfile = Field(..., description="Risk profile")
    trading_strategies: List[TradingStrategy] = Field(
        default_factory=list, description="Active trading strategies"
    )
    ...
    def to_agent_context(
        ...
        if permission_level in [
            ...
            AgentPermissionLevel.READ_WRITE,
            ...
            base_context.update(
                {
                    "portfolio_settings": {
                        ...
                        "max_position_size": float(
                            self.portfolio_settings.max_position_size
                        ),
                    },
                    "active_strategies": [
            ...
            base_context.update(
                {
                    "risk_profile": {
                        ...
                        "max_portfolio_drawdown": float(
                            self.risk_profile.max_portfolio_drawdown
                        ),
    ...
    def has_permission_for_agent(
        self, agent_name: str, required_permission: str
    ) -> bool:
        """
        ...
        return agent_name in known_agents and required_permission in basic_permissions
