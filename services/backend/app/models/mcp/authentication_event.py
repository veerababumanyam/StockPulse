...
class AuthenticationEvent(BaseModel):
    ...
    user_context: Optional[UserContext] = Field(None, description="User context at event time")
...
class AgentNotificationEvent(BaseModel):
    ...
    user_id: str = Field(..., description="User identifier")
    event_type: AuthenticationEventType = Field(..., description="Authentication event type")
    ...
    status: AgentNotificationStatus = Field(default=AgentNotificationStatus.PENDING, description="Notification status")
    ...
    def is_retryable(self) -> bool:
        ...
        return (
            self.status == AgentNotificationStatus.FAILED and 
...
class EventStatistics(BaseModel):
    ...
    total_agent_notifications: int = Field(default=0, description="Total agent notifications")
    successful_notifications: int = Field(default=0, description="Successful notifications")
    ...
    events_by_agent: Dict[str, int] = Field(default_factory=dict, description="Notifications grouped by agent")
